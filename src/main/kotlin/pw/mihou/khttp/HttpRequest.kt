...
class HttpRequest(
    ...
    suspend fun response(): HttpResponse {
        return withContext(Dispatchers.IO) {
            ...
            val connectionBuilder = HttpRequest.newBuilder()
                .uri(uri)
                .version(options.httpVersion)
                .method(method.value, if (body !=  null) HttpRequest.BodyPublishers.ofString(body) else HttpRequest.BodyPublishers.noBody())
            ...
            val connection = Client.send(
                connectionBuilder.build(),
                if (options.expectBody)  java.net.http.HttpResponse.BodyHandlers.ofString()
                else java.net.http.HttpResponse.BodyHandlers.discarding()
            )
            @Suppress("UNCHECKED_CAST")
            mutableResponse.body =
                if (options.expectBody) (connection as java.net.http.HttpResponse<String>).body()
                else ""
    ...
    private val Map<String, Any>.asQueryParameters: String get() =
